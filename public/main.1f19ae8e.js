...
let activeEffect;
const ITERATE_KEY = exports.ITERATE_KEY = Symbol(!!("development" !== "production") ? "iterate" : "");
...
function trigger(target, type, key, newValue, oldValue, oldTarget) {
  ...
  if (type === "clear") {
    ...
    depsMap.forEach((dep, key2) => {
      if (key2 === "length" || !(0, _shared.isSymbol)(key2) && key2 >= newLength) {
        deps.push(dep);
      }
    });
  } else {
...
function triggerEffect(effect2, debuggerEventExtraInfo) {
  if (effect2 !== activeEffect || effect2.allowRecurse) {
    if (!!("development" !== "production") && effect2.onTrigger) {
...
function triggerRef(ref2) {
  triggerRefValue(ref2, !!("development" !== "production") ? ref2.value : void 0);
}
function unref(ref2) {
...
exports.Comment = exports.BaseTransitionPropsValidators = exports.BaseTransition = void 0;
...
let devtools = exports.devtools = void 0;
let buffer = [];
...
function getKeepAliveChild(vnode) {
  ...
  !!("development" !== "production") && vnode.component ? vnode.component.subTree : vnode.children ? vnode.children[0] : void 0 : vnode;
}
...
const PublicInstanceProxyHandlers = {
  ...
  set({
    ...
    if (hasSetupBinding(setupState, key)) {
      ...
      !!("development" !== "production") && warn(`Attempting to mutate prop "${key}". Props are readonly.`);
      return false;
    }
    if (key[0] === "$" && key.slice(1) in instance) {
      !!("development" !== "production") && warn(`Attempting to mutate public property "${key}". Properties starting with $ are reserved and readonly.`);
      return false;
    } else {
...
function useModel(props, name, options) {
  ...
  if (!!("development" !== "production") && !i.propsOptions[0][name]) {
...
function updateProps(instance, rawProps, rawPrevProps, optimized) {
  ...
  if (hasAttrsChanged) {
    (0, _reactivity.trigger)(instance, "set", "$attrs");
  }
  if (!!("development" !== "production")) {
    validateProps(rawProps || {}, props, instance);
  }
...
function baseCreateRenderer(options, createHydrationFns) {
  ...
  const setupRenderEffect = (instance, initialVNode, container, anchor, parentSuspense, isSVG, optimized) => {
    ...
    const effect = instance.effect = new _reactivity.ReactiveEffect(componentUpdateFn, () => queueJob(update), instance.scope
...
function isSameVNodeType(n1, n2) {
  if (!!("development" !== "production") && n2.shapeFlag & 6 && hmrDirtyComponents.has(n2.type)) {
    n1.shapeFlag &= ~256;
    n2.shapeFlag &= ~512;
    return false;
  }
...
{
  if (!(globalCurrentInstanceSetters = (0, _shared.getGlobalThis)()[settersKey])) {
    globalCurrentInstanceSetters = (0, _shared.getGlobalThis)()[settersKey] = [];
  }
...
function getAttrsProxy(instance) {
  return instance.attrsProxy || (instance.attrsProxy = new Proxy(instance.attrs, !!("development" !== "production") ? {
    get(target, key) {
      markAttrsAccessed();
...
const useSSRContext = () => {
  {
    ...
    if (!ctx) {
      !!("development" !== "production") && warn(`Server rendering context not provided. Make sure to only call useSSRContext() conditionally in the server build.`);
    }
    return ctx;
  }
};
...
const version = exports.version = "3.3.12";
const _ssrUtils = {
...
const TransitionPropsValidators = Transition.props = /* @__PURE__ */(0, _shared.extend)({}, _runtimeCore.BaseTransitionPropsValidators, DOMTransitionPropsValidators);
const callHook = (hook, args = []) => {
  if ((0, _shared.isArray)(hook)) {
...
const vModelText = exports.vModelText = {
  ...
  beforeUpdate(el, {
    ...
    const elValue = number || el.type === "number" ? (0, _shared.looseToNumber)(el.value) : el.value;
    const newValue = value == null ? "" : value;
    if (elValue === newValue) {
      return;
    }
...
exports.isStaticExp = void 0;
exports.isStaticPropertyKey = exports.isStaticProperty = void 0;
exports.isTemplateNode = isTemplateNode;
exports.isText = isText$1;
exports.isVSlot = isVSlot;
exports.noopDirectiveTransform = exports.locStub = void 0;
...
function defaultOnWarn(msg) {
  !!("development" !== "production") && console.warn(`[Vue warn] ${msg.message}`);
}
function createCompilerError(code, loc, messages, additionalMessage) {
  const msg = !!("development" !== "production") || false ? (messages || errorMessages)[code] + (additionalMessage || ``) : code;
...
const FRAGMENT = exports.FRAGMENT = Symbol(!!("development" !== "production") ? `Fragment` : ``);
const TELEPORT = exports.TELEPORT = Symbol(!!("development" !== "production") ? `Teleport` : ``);
const SUSPENSE = exports.SUSPENSE = Symbol(!!("development" !== "production") ? `Suspense` : ``);
const KEEP_ALIVE = exports.KEEP_ALIVE = Symbol(!!("development" !== "production") ? `KeepAlive` : ``);
const BASE_TRANSITION = exports.BASE_TRANSITION = Symbol(!!("development" !== "production") ? `BaseTransition` : ``);
const OPEN_BLOCK = exports.OPEN_BLOCK = Symbol(!!("development" !== "production") ? `openBlock` : ``);
const CREATE_BLOCK = exports.CREATE_BLOCK = Symbol(!!("development" !== "production") ? `createBlock` : ``);
const CREATE_ELEMENT_BLOCK = exports.CREATE_ELEMENT_BLOCK = Symbol(!!("development" !== "production") ? `createElementBlock` : ``);
const CREATE_VNODE = exports.CREATE_VNODE = Symbol(!!("development" !== "production") ? `createVNode` : ``);
const CREATE_ELEMENT_VNODE = exports.CREATE_ELEMENT_VNODE = Symbol(!!("development" !== "production") ? `createElementVNode` : ``);
const CREATE_COMMENT = exports.CREATE_COMMENT = Symbol(!!("development" !== "production") ? `createCommentVNode` : ``);
const CREATE_TEXT = exports.CREATE_TEXT = Symbol(!!("development" !== "production") ? `createTextVNode` : ``);
const CREATE_STATIC = exports.CREATE_STATIC = Symbol(!!("development" !== "production") ? `createStaticVNode` : ``);
const RESOLVE_COMPONENT = exports.RESOLVE_COMPONENT = Symbol(!!("development" !== "production") ? `resolveComponent` : ``);
const RESOLVE_DYNAMIC_COMPONENT = exports.RESOLVE_DYNAMIC_COMPONENT = Symbol(!!("development" !== "production") ? `resolveDynamicComponent` : ``);
const RESOLVE_DIRECTIVE = exports.RESOLVE_DIRECTIVE = Symbol(!!("development" !== "production") ? `resolveDirective` : ``);
const RESOLVE_FILTER = exports.RESOLVE_FILTER = Symbol(!!("development" !== "production") ? `resolveFilter` : ``);
const WITH_DIRECTIVES = exports.WITH_DIRECTIVES = Symbol(!!("development" !== "production") ? `withDirectives` : ``);
const RENDER_LIST = exports.RENDER_LIST = Symbol(!!("development" !== "production") ? `renderList` : ``);
const RENDER_SLOT = exports.RENDER_SLOT = Symbol(!!("development" !== "production") ? `renderSlot` : ``);
const CREATE_SLOTS = exports.CREATE_SLOTS = Symbol(!!("development" !== "production") ? `createSlots` : ``);
const TO_DISPLAY_STRING = exports.TO_DISPLAY_STRING = Symbol(!!("development" !== "production") ? `toDisplayString` : ``);
const MERGE_PROPS = exports.MERGE_PROPS = Symbol(!!("development" !== "production") ? `mergeProps` : ``);
const NORMALIZE_CLASS = exports.NORMALIZE_CLASS = Symbol(!!("development" !== "production") ? `normalizeClass` : ``);
const NORMALIZE_STYLE = exports.NORMALIZE_STYLE = Symbol(!!("development" !== "production") ? `normalizeStyle` : ``);
const NORMALIZE_PROPS = exports.NORMALIZE_PROPS = Symbol(!!("development" !== "production") ? `normalizeProps` : ``);
const GUARD_REACTIVE_PROPS = exports.GUARD_REACTIVE_PROPS = Symbol(!!("development" !== "production") ? `guardReactiveProps` : ``);
const TO_HANDLERS = exports.TO_HANDLERS = Symbol(!!("development" !== "production") ? `toHandlers` : ``);
const CAMELIZE = exports.CAMELIZE = Symbol(!!("development" !== "production") ? `camelize` : ``);
const CAPITALIZE = exports.CAPITALIZE = Symbol(!!("development" !== "production") ? `capitalize` : ``);
const TO_HANDLER_KEY = exports.TO_HANDLER_KEY = Symbol(!!("development" !== "production") ? `toHandlerKey` : ``);
const SET_BLOCK_TRACKING = exports.SET_BLOCK_TRACKING = Symbol(!!("development" !== "production") ? `setBlockTracking` : ``);
const PUSH_SCOPE_ID = exports.PUSH_SCOPE_ID = Symbol(!!("development" !== "production") ? `pushScopeId` : ``);
const POP_SCOPE_ID = exports.POP_SCOPE_ID = Symbol(!!("development" !== "production") ? `popScopeId` : ``);
const WITH_CTX = exports.WITH_CTX = Symbol(!!("development" !== "production") ? `withCtx` : ``);
const UNREF = exports.UNREF = Symbol(!!("development" !== "production") ? `unref` : ``);
const IS_REF = exports.IS_REF = Symbol(!!("development" !== "production") ? `isRef` : ``);
const WITH_MEMO = exports.WITH_MEMO = Symbol(!!("development" !== "production") ? `withMemo` : ``);
const IS_MEMO_SAME = exports.IS_MEMO_SAME = Symbol(!!("development" !== "production") ? `isMemoSame` : ``);
const helperNameMap = exports.helperNameMap = {
  [FRAGMENT]: `Fragment`,
  [TELEPORT]: `Teleport`,
  [SUSPENSE]: `Suspense`,
...
const isMemberExpressionNode = exports.isMemberExpressionNode = _shared.NOOP;
const isMemberExpression = exports.isMemberExpression = isMemberExpressionBrowser;
...
function findDir(node, name, allowEmpty = false) {
  for (let i = 0; i < node.props.length; i++) {
    ...
    if (p.type === 7 && (allowEmpty || p.exp) && ((0, _shared.isString)(name) ? p.name === name : name.test(p.name))) {
      return p;
    }
  }
}
...
const forAliasRE = exports.forAliasRE = /([\s\S]*?)\s+(?:in|of)\s+([\s\S]*)/;
...
function checkCompatEnabled(key, context, loc, ...args) {
  ...
  if (!!("development" !== "production") && enabled) {
      ...
      if (!context.inVPre && startsWith(s, context.options.delimiters[0])) {
        ...
        if (s.length === 1) {
          ...
          if (isCompatEnabled("COMPILER_NATIVE_TEMPLATE", context) && node && node.tag === "template" && !node.props.some(p => p.type === 7 && isSpecialTemplateDirective(p.name))) {
            !!("development" !== "production") && warnDeprecation("COMPILER_NATIVE_TEMPLATE", context, node.loc);
            node = node.children;
          }
        } else if (s[1] === "?") {
...
function parseTag(context, type, parent) {
  ...
  if (!!("development" !== "production") && isCompatEnabled("COMPILER_V_IF_V_FOR_PRECEDENCE", context)) {
    let hasIf = false;
    let hasFor = false;
...
function parseAttribute(context, nameSet) {
  ...
  if (!context.inVPre && /^(v-[A-Za-z0-9-]|:|\.|@|#)/.test(name)) {
    ...
    if (dirName === "bind" && arg) {
      ...
      if (!!("development" !== "production") && modifiers.includes("prop")) {
...
function walk(node, context, doNotHoistNode = false) {
  ...
  for (let i = 0; i < children.length; i++) {
    ...
    if (child.type === 1 && child.tagType === 0) {
      ...
      if (constantType > 0) {
        if (constantType >= 2) {
          child.codegenNode.patchFlag = -1 + (!!("development" !== "production") ? ` /* HOISTED */` : ``);
  ...
  if (hoistedCount && hoistedCount === originalCount && node.type === 1 && node.tagType === 0 && node.codegenNode && node.codegenNode.type === 13 && (0, _shared.isArray)(node.codegenNode.children)) {
...
function createRootCodegen(root, context) {
  ...
  if (children.length === 1) {
    ...
    let patchFlag = 64;
    let patchFlagText = _shared.PatchFlagNames[64];
    if (!!("development" !== "production") && children.filter(c => c.type !== 3).length === 1) {
      patchFlag |= 2048;
      patchFlagText += `, ${_shared.PatchFlagNames[2048]}`;
    }
    root.codegenNode = createVNodeCall(context, helper(FRAGMENT), void 0, root.children, patchFlag + (!!("development" !== "production") ? ` /* ${patchFlagText} */` : ``), void 0, void 0, true, void 0, false
    /* isComponent */);
  } else ;
}
...
function genNodeListAsArray(nodes, context) {
  const multilines = nodes.length > 3 || !!("development" !== "production") && nodes.some(n => (0, _shared.isArray)(n) || !isText(n));
  context.push(`[`);
...
const TS_NODE_TYPES = exports.TS_NODE_TYPES = ["TSAsExpression",
// foo as number
"TSTypeAssertion",
// (<number>foo)
"TSNonNullExpression",
  ...
  if (dir.name === "if") {
    ...
    while (i-- >= -1) {
      ...
      if (sibling && sibling.type === 3) {
        ...
        !!("development" !== "production") && comments.unshift(sibling);
        continue;
      }
...
function createChildrenCodegenNode(branch, keyIndex, context) {
  ...
  if (needFragmentWrapper) {
    if (children.length === 1 && firstChild.type === 11) {
      ...
      let patchFlag = 64;
      let patchFlagText = _shared.PatchFlagNames[64];
      if (!!("development" !== "production") && !branch.isTemplateIf && children.filter(c => c.type !== 3).length === 1) {
        patchFlag |= 2048;
        patchFlagText += `, ${_shared.PatchFlagNames[2048]}`;
      }
      return createVNodeCall(context, helper(FRAGMENT), createObjectExpression([keyProperty]), children, patchFlag + (!!("development" !== "production") ? ` /* ${patchFlagText} */` : ``), void 0, void 0, true, false, false, branch.loc);
    }
  } else {
...
const transformFor = createStructuralDirectiveTransform("for", (node, dir, context) => {
  ...
  return processFor(node, dir, context, forNode => {
    ...
    forNode.codegenNode = createVNodeCall(context, helper(FRAGMENT), void 0, renderExp, fragmentFlag + (!!("development" !== "production") ? ` /* ${_shared.PatchFlagNames[fragmentFlag]} */` : ``), void 0, void 0, true, !isStableFragment, false, node.loc);
    return () => {
      let childBlock;
      const {
        children
      } = forNode;
      if ((!!("development" !== "production") || false) && isTemplate) {
      ...
      if (slotOutlet) {
        ...
        childBlock = createVNodeCall(context, helper(FRAGMENT), keyProperty ? createObjectExpression([keyProperty]) : void 0, node.children, 64 + (!!("development" !== "production") ? ` /* ${_shared.PatchFlagNames[64]} */` : ``), void 0, void 0, true, void 0, false
        /* isComponent */);
      } else {
...
function buildSlots(node, context, buildSlotFn = buildClientSlotFn) {
  ...
  createSimpleExpression(slotFlag + (!!("development" !== "production") ? ` /* ${_shared.slotFlagsText[slotFlag]} */` : ``), false))), loc);
    ...
    if (p.type === 6) {
      ...
      if (p.name === "bind" && isStaticArgOf(p.arg, "name")) {
        ...
        if (p.name === "bind" && p.arg && isStaticExp(p.arg)) {
          p.arg.content = (0, _shared.camelize)(p.arg.content);
        }
        nonNameProps.push(p);
      }
    }
...
const transformOn = (dir, node, context, augmentor) => {
  ...
  if (arg.type === 4) {
    if (arg.isStatic) {
      ...
      if (!!("development" !== "production") && rawName.startsWith("vnode")) {
      ...
      (0, _shared.toHandlerKey)((0, _shared.camelize)(rawName)) :
      ...
      for (let i = 0; i < children.length; i++) {
        ...
        if (isText$1(child) || child.type === 8) {
          ...
          if (!context.ssr && getConstantType(child, context) === 0) {
            callArgs.push(1 + (!!("development" !== "production") ? ` /* ${_shared.PatchFlagNames[1]} */` : ``));
          }
          children[i] = {
            type: 12,
            content: child,
...
const transformModel = (dir, node, context) => {
  ...
  const eventName = arg ? isStaticExp(arg) ? `onUpdate:${(0, _shared.camelize)(arg.content)}` : createCompoundExpression(['"onUpdate:" + ', arg]) : `onUpdate:modelValue`;
  let assignmentExp;
...
function parseFilter(node, context) {
  ...
  if (filters.length) {
    !!("development" !== "production") && warnDeprecation("COMPILER_FILTER", context, node.loc);
...
function getBaseTransformPreset(prefixIdentifiers) {
  return [[transformOnce, transformIf, transformMemo, transformFor, ...[transformFilter], ...(!!("development" !== "production") ? [transformExpression] : []), transformSlotOutlet, transformElement, trackSlotScopes, transformText], {
    on: transformOn,
    bind: transformBind,
...
},{"@vue/shared":"../../node_modules/@vue/shared/dist/shared.esm-bundler.js"}],"../../node_modules/@vue/compiler-dom/dist/compiler-dom.esm-bundler.js":[function(require,module,exports) {
"use strict";
...
var _shared = require("@vue/shared");
const V_MODEL_RADIO = exports.V_MODEL_RADIO = Symbol(!!("development" !== "production") ? `vModelRadio` : ``);
const V_MODEL_CHECKBOX = exports.V_MODEL_CHECKBOX = Symbol(!!("development" !== "production") ? `vModelCheckbox` : ``);
const V_MODEL_TEXT = exports.V_MODEL_TEXT = Symbol(!!("development" !== "production") ? `vModelText` : ``);
const V_MODEL_SELECT = exports.V_MODEL_SELECT = Symbol(!!("development" !== "production") ? `vModelSelect` : ``);
const V_MODEL_DYNAMIC = exports.V_MODEL_DYNAMIC = Symbol(!!("development" !== "production") ? `vModelDynamic` : ``);
const V_ON_WITH_MODIFIERS = exports.V_ON_WITH_MODIFIERS = Symbol(!!("development" !== "production") ? `vOnModifiersGuard` : ``);
const V_ON_WITH_KEYS = exports.V_ON_WITH_KEYS = Symbol(!!("development" !== "production") ? `vOnKeysGuard` : ``);
const V_SHOW = exports.V_SHOW = Symbol(!!("development" !== "production") ? `vShow` : ``);
const TRANSITION = exports.TRANSITION = Symbol(!!("development" !== "production") ? `Transition` : ``);
const TRANSITION_GROUP = exports.TRANSITION_GROUP = Symbol(!!("development" !== "production") ? `TransitionGroup` : ``);
...
function createDOMCompilerError(code, loc) {
  return (0, _compilerCore.createCompilerError)(code, loc, !!("development" !== "production") || false ? DOMErrorMessages : void 0);
}
...
const transformModel = (dir, node, context) => {
  ...
  function checkDuplicatedValue() {
    ...
    if (value && (0, _compilerCore.isStaticArgOf)(value.arg, "value")) {
...
const isNonKeyModifier = /* @__PURE__ */(0, _shared.makeMap)(
...
const resolveModifiers = (key, modifiers, context, loc) => {
  ...
  for (let i = 0; i < modifiers.length; i++) {
    ...
    if (modifier === "native" && (0, _compilerCore.checkCompatEnabled)("COMPILER_V_ON_NATIVE", context, loc)) {
...
const transformClick = (key, event) => {
  ...
  return isStaticClick ? (0, _compilerCore.createSimpleExpression)(event, true) : key.type !== 4 ? (0, _compilerCore.createCompoundExpression)([`(`, key, `) === "onClick" ? "${event}" : (`, key, `)`]) : key;
};
...
const ignoreSideEffectTags = (node, context) => {
  if (node.type === 1 && node.tagType === 0 && (node.tag === "script" || node.tag === "style")) {
    !!("development" !== "production") && context.onError(createDOMCompilerError(63, node.loc));
    context.removeNode();
  }
};
const DOMNodeTransforms = exports.DOMNodeTransforms = [transformStyle, ...(!!("development" !== "production") ? [transformTransition] : [])];
const DOMDirectiveTransforms = exports.DOMDirectiveTransforms = {
...
var _runtimeDom = _interopRequireWildcard(require("@vue/runtime-dom"));
...
function getCache(options) {
  let c = compileCache.get(options != null ? options : _shared.EMPTY_OBJ);
  if (!c) {
    c = /* @__PURE__ */Object.create(null);
    compileCache.set(options != null ? options : _shared.EMPTY_OBJ, c);
  }
  return c;
}
function compileToFunction(template, options) {
  if (!(0, _shared.isString)(template)) {
    if (template.nodeType) {
      ...
      !!("development" !== "production") && (0, _runtimeDom.warn)(`invalid template option: `, template);
      return _shared.NOOP;
    }
  }
  const key = template;
  ...
  if (template[0] === "#") {
    ...
    if (!!("development" !== "production") && !el) {
      (0, _runtimeDom.warn)(`Template element not found or is empty: ${template}`);
    }
    template = el ? el.innerHTML : ``;
  }
  const opts = (0, _shared.extend)({
    hoistStatic: true,
    onError: !!("development" !== "production") ? onError : void 0,
    onWarn: !!("development" !== "production") ? e => onError(e, true) : _shared.NOOP
  }, options);
  ...
  const {
    code
  } = (0, _compilerDom.compile)(template, opts);
...
},{"@vue/runtime-dom":"../../node_modules/@vue/runtime-dom/dist/runtime-dom.esm-bundler.js","@vue/compiler-dom":"../../node_modules/@vue/compiler-dom/dist/compiler-dom.esm-bundler.js","@vue/shared":"../../node_modules/@vue/shared/dist/shared.esm-bundler.js"}],"../components/DarkMode.js":[function(require,module,exports) {
"use strict";
...
var _vue = require("vue/dist/vue.esm-bundler");
var _default = exports.default = {
  setup: function setup() {
    var currentTheme = (0, _vue.computed)({
      get: function get() {
        if (localStorage.getItem('theme') || window.matchMedia('(prefers-color-scheme: dark)').matches) return 1;
        return 0;
      },
      set: function set(value) {
        if (value) localStorage.setItem('theme', 1);else localStorage.removeItem('theme');
      }
    });
    return {
...
},{"vue/dist/vue.esm-bundler":"../../node_modules/vue/dist/vue.esm-bundler.js"}],"../components/RootLevel/Header.js":[function(require,module,exports) {
"use strict";
...
var initHeader = function initHeader() {
  var headerApp = (0, _vue.createApp)({
    template: "\n      <nav>\n        <ul style=\"display: flex; list-style: none; gap: 10px; margin: 0; padding: 0;\">\n          <li>a</li>\n          <li>b</li>\n          <li>c</li>\n        </ul>\n      </nav>\n      <dark-mode />\n    "
  });
  headerApp.component('dark-mode', _DarkMode.default);
  headerApp.mount('#header');
  return headerApp;
};
var _default = exports.default = initHeader;
},{"vue/dist/vue.esm-bundler":"../../node_modules/vue/dist/vue.esm-bundler.js","../DarkMode":"../components/DarkMode.js"}],"../components/RootLevel/Main.js":[function(require,module,exports) {
"use strict";
...
var _default = exports.default = initHeader;
},{"vue/dist/vue.esm-bundler":"../../node_modules/vue/dist/vue.esm-bundler.js"}],"main.js":[function(require,module,exports) {
"use strict";

var _Header = _interopRequireDefault(require("../components/RootLevel/Header"));
var _Main = _interopRequireDefault(require("../components/RootLevel/Main"));
function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }
window.__VUE_OPTIONS_API__ = false; // eslint-disable-line no-underscore-dangle
window.__VUE_PROD_DEVTOOLS__ = false; // eslint-disable-line no-underscore-dangle

(0, _Header.default)();
(0, _Main.default)();
},{"../components/RootLevel/Header":"../components/RootLevel/Header.js","../components/RootLevel/Main":"../components/RootLevel/Main.js"}],"../../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
var global = arguments[3];
